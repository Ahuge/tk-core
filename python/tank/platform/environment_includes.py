...
def _merge_included_data(data, include_data):
    ...
    for data_key in data:
        if data_key not in include_data:
            processed_data[data_key] = data[data_key]
...
def _merge_included_data_r(data, include):
    ...
    if isinstance(current_data, list) and isinstance(included_data, list):
        processed_val = []
        processed_val.extend(current_data)
        processed_val.extend(included_data)

    elif isinstance(current_data, dict) and isinstance(included_data, dict):
        processed_val = {}
